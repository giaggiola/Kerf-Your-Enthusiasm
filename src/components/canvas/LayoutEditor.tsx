...
export default function LayoutEditor({
  ...
  return (
    <div className="flex min-h-full flex-col overflow-hidden border border-[var(--line)] bg-white">

      {/* ── Toolbar ─────────────────────────────────────────────────── */}
      <div className="flex flex-shrink-0 items-center gap-1.5 border-b border-[var(--line)] bg-white px-3 py-2">
        ...
        {/* Zoom */}
        <div className="flex items-center overflow-hidden rounded border border-[var(--line)] bg-[#fafaf8] text-xs text-[var(--muted)]">
          <button
        ...
        <button
          onClick={onReoptimize}
            className="rounded bg-[var(--accent)] px-2.5 py-1.5 text-xs font-medium text-white transition-colors hover:bg-[var(--accent-dark)]"
        >↻ Re-optimize</button>
      </div>

      {/* ── Sheet grid — wrapping row layout on slate-100 surface ─── */}
      <div className="overflow-auto bg-[#f3f3f0]">
        <div className="flex min-w-fit flex-wrap gap-5 p-4">
          {result.sheets.map((sheet, i) => {
            const merged = mergedSheets[i];
            ...
            return (
              <div key={i} className="flex flex-col">
                ...
                <div
                  ...
                  className={`shadow-sm ring-1 select-none flex-shrink-0 transition-shadow ${
      ...
      {(() => {
        ...
        return (
          <div className="border-t border-slate-200 bg-[#fafaf8] px-4 py-2.5 flex-shrink-0">
            <div className="mb-1.5 text-[10px] text-slate-400">Materials needed</div>
