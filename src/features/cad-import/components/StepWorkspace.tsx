...
const StepViewer3D = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-full h-full flex items-center justify-center text-neutral-400 text-sm">Loading 3D…</div> }
);
...
export default function StepWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const restoreWorkspace = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) router.push('/cut-list');
          return;
        }
        ...
        const displayUnits = localStorage.getItem('kerf-parts-display-unit');
        setUnits(displayUnits === '"in"' ? 'in' : displayUnits === '"mm"' ? 'mm' : nextUnits);
        ...
        const sourceFile = sourceParams.get('file');
        const sourceBody = Number(sourceParams.get('body'));
        if (sourceFile && Number.isInteger(sourceBody)) {
          const sourceSession = restoredSessions.find(item => item.stepFileId === sourceFile);
          const bodyIndex = sourceSession?.bodyStates.findIndex(item => item.body.index === sourceBody) ?? -1;
          if (sourceSession && bodyIndex >= 0) sourceSession.selectedBodyIdx = bodyIndex;
        }
        const activeStepFileId = sourceFile ?? data.stepActiveFileId ?? null;
  ...
  const handleAddToCutList = async () => {
    ...
    setAdding(true);
    try {
      const { stepFileId, sessionId, bodyStates, selectedFaceIndices } = activeSession;
      const toAdd = bodyStates.filter((bs) => bs.included && bs.confirmed);
      const cutsPayload = await Promise.all(toAdd.map(async (bs) => {
        const bodyArrayIdx = bodyStates.indexOf(bs);

        const faceIdx =
          selectedFaceIndices[bodyArrayIdx] ??
          bs.body.faces.find((f) => f.is_top_face)?.index ?? 0;
        const preview = await fetch(`/api/v1/step/${sessionId}/preview`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ body_index: bs.body.index, face_index: faceIdx }),
        });
        if (!preview.ok) throw new Error(`Could not prepare ${bs.name}. Review its cut face and try again.`);
        const dims: [number, number, number] | null = (await preview.json()).face_dims_mm;
        if (!dims || !dims.every(v => Number.isFinite(v) && v > 0)) throw new Error(`${bs.name} has no usable cutting dimensions.`);
      ...
      await persistWorkspaceToProject(sessions, activeIdx);
      ...
      setUploadError(e instanceof Error ? e.message : 'Could not add these parts. Please try again.');
    } finally {
      setAdding(false);
    }
  };
  ...
  const dropzone = (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label="Choose STEP files"
      onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); fileInputRef.current?.click(); } }}
      onClick={() => fileInputRef.current?.click()}
      className={`group rounded-none border border-dashed px-6 py-14 text-center cursor-pointer transition-all ${
        dragOver
          ? 'border-[var(--accent)] bg-[var(--accent-soft)] '
          : 'border-[#c9cdc6] bg-white hover:border-[var(--accent)] hover:bg-[#f5f5f5]'
      }`}
    >
      <input ref={fileInputRef} type="file" accept=".step,.stp" multiple className="hidden" onChange={onFileInput} />
      {uploading ? (
        <>
          <p className="font-medium text-[var(--ink)]">{uploadProgress ?? 'Parsing STEP files…'}</p>
          <p className="mt-1 text-sm text-[var(--muted)]">Complex assemblies can take a moment.</p>
        </>
      ) : (
        <>
          <p className="font-medium text-[var(--ink)]">Drop STEP files here</p>
          <p className="mt-1 text-sm text-[var(--muted)]">or browse your computer · .step and .stp supported</p>
        </>
      )}
      {uploadError && <p className="mx-auto mt-4 max-w-md text-sm text-neutral-600">{uploadError}</p>}
    </div>
  );
  ...
  if (restoring) {
    return (
      <div className="flex h-full items-center justify-center gap-3 text-sm text-[var(--muted)]">
  ...
  if (sessions.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-5 py-10 sm:py-14">
        ...
        <div className="mb-8">
          <p className="app-eyebrow">Import 3D</p>
          <h1 className="mt-2 text-3xl font-normal tracking-[-0.04em] text-[var(--ink)]">Choose your parts</h1>
          <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--muted)]">
            Upload a STEP model, then confirm the face of each part.
          </p>
        </div>
        {dropzone}
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#f5f5f5]">
      ...
      <div className="shrink-0 border-b border-[var(--line)] bg-white">
        <div className="flex min-h-[64px] items-center justify-between gap-4 px-4 lg:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <Link href={`/projects/${id}`} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-none border border-[var(--line)] text-[var(--muted)] hover:bg-black/[0.03] hover:text-[var(--ink)]" aria-label="Back to project">
              ←
            </Link>
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-[var(--ink)]">{projectName}</p>
              <p className="text-xs text-[var(--muted)]">Import 3D</p>
            </div>
          </div>
          ...
          {addingNew ? (
            ...
            <div className="flex items-center gap-2">
              <div className="hidden overflow-hidden rounded-none border border-[var(--line)] text-xs sm:flex">
              ...
              >
                ...
                <span className="hidden sm:inline">{adding ? 'Adding…' : `Add ${confirmedCount || ''} part${confirmedCount === 1 ? '' : 's'}`}</span>
                <span>→</span>
              </button>
            </div>
          )}
        ...
        <div className="flex min-h-11 items-center gap-3 border-t border-[var(--line)] px-4 lg:px-6">
          ...
          <div className="flex min-w-0 items-center gap-1.5 overflow-x-auto py-1.5">
            {sessions.map((s, i) => (
              <div
                key={s.sessionId}
                className={`flex shrink-0 select-none items-center gap-1 rounded-none border px-2.5 py-1.5 text-xs transition-colors ${
              ...
              >
                ...
                <button
                  ...
                  className={`ml-1 flex h-4 w-4 items-center justify-center rounded-none leading-none transition-colors ${
                    i === activeIdx && !addingNew ? 'hover:bg-white/20' : 'hover:bg-neutral-300'
                  }`}
            ...
            <button
              ...
              className={`shrink-0 rounded-none border px-2.5 py-1.5 text-xs font-medium transition-colors ${
                addingNew
                  ? 'border-[var(--ink)] bg-[var(--ink)] text-white'
      ...
      {addedCount !== null && !addingNew && (
        <div className="flex shrink-0 items-center justify-between border-b border-neutral-200 bg-neutral-50 px-4 py-2.5 lg:px-6">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-neutral-600 shrink-0" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <span className="text-sm text-neutral-800 font-medium">
              {addedCount} part{addedCount !== 1 ? 's are' : ' is'} ready in your cut list.
            </span>
          </div>
          <button
      ...
      {uploadError && !addingNew && <p className="parts-notice px-4" role="alert">{uploadError}</p>}
      ...
      {addingNew ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto lg:flex-row lg:overflow-hidden">
          ...
          {/* Center: 3D viewer */}
          <div className="relative min-h-[360px] min-w-0 flex-1 bg-[#f5f5f5]">
            ...
            {activeMeshData.length > 0 && (
              <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-none border border-black/5 bg-white/85 px-3 py-1.5 text-[11px] text-[var(--muted)] shadow-sm backdrop-blur">
                Orbit: drag · Pan: shift-drag · Select: click a face
              </div>
            )}
            {activeMeshData.length > 0 && (
              <div className="pointer-events-none absolute left-4 top-4 max-w-56 rounded-none border border-white/70 bg-white/85 p-3 shadow-sm backdrop-blur">
                <p className="text-xs font-medium text-[var(--ink)]">Choose the sheet face</p>
          ...
          >
            <div className="flex flex-col gap-[3px] pointer-events-none">
              {[0, 1, 2, 3, 4].map((i) => (
                <div key={i} className="h-[3px] w-[3px] rounded-none bg-[#b6bbb3] group-hover/sep:bg-[var(--accent)]" />
              ))}
            </div>
          </div>
                ...
                <div className="mb-3 flex shrink-0 items-start gap-2 border-b border-[var(--line)] pb-3">
                  ...
                  {!bodyStates[selectedBodyIdx]?.confirmed ? (
                    <button
                      ...
                      className="shrink-0 rounded-none bg-[#f5f5f5] px-2.5 py-1.5 text-xs font-medium text-neutral-700 hover:bg-[#f5f5f5]"
                    >
                      Confirm face
                    </button>
                  ) : (
                    <span className="flex shrink-0 items-center gap-1 rounded-none bg-[#f5f5f5] px-2 py-1 text-[10px] font-medium text-neutral-700">
