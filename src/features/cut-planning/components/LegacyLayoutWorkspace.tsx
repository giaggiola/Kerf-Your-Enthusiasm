...
import { dimensionsToLayout, dimensionsFromLayout, layoutDimension, layoutOverridesInUnits } from '../domain/units';
import { useLocalStorage } from '@/hooks/useLocalStorage';
import { LayoutDimensionInput } from './LayoutDimensionInput';
...
function getPartThicknessDisplay(
  ...
  if (cut.t > 0) {
    return units === 'mm' ? inToMM(cut.t).toFixed(1) : toFraction(cut.t);
  }
  if (sheetThickness > 0) {
    return units === 'mm' ? inToMM(sheetThickness).toFixed(1) : toFraction(sheetThickness);
  }
  return '';
}
...
function CutRow({
  ...
  const commitT = () => {
    setEditingT(false);
    const entered = parseFloat(tDraft);
    const v = layoutDimension(entered, units, 'in');
    if (!isNaN(v) && v >= 0 && v !== cut.t) onChange({ ...cut, t: v });
    else setTDraft(String(layoutDimension(cut.t, 'in', units)));
  };
      ...
      <div className="flex flex-wrap items-center gap-1.5 px-1 pb-1.5 pl-6">
        ...
        {editingT ? (
          <input
            ...
            onBlur={commitT}
            onKeyDown={(e) => { if (e.key === 'Enter') commitT(); if (e.key === 'Escape') { setEditingT(false); setTDraft(String(layoutDimension(cut.t, 'in', units))); } }}
          ...
          <button
            onClick={() => { setEditingT(true); setTDraft(String(layoutDimension(cut.t, 'in', units))); }}
...
function GroupCard({
  name, items, dim, units, isDropTarget,
  ...
  name: string; items: CutType[]; dim: (v: number) => string; units: UnitSystem; isDropTarget: boolean;
...
export default function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [units, setUnits] = useLocalStorage<UnitSystem>('kerf-parts-display-unit', 'mm');
  ...
  }, [cuts, excludedKeys, groupMultipliers, kerf, layoutEditorHydrated, loading, overrides, padding, result, stocks]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function fetchProject() {
    try {
      ...
      if (res.ok) {
        ...
        setProject(data);
        const storageUnits = data.units === 'mm' ? 'mm' : 'in';
        setStocks(data.stocks.map((stock, index) => toAppStock(dimensionsToLayout(stock, storageUnits), index)));
        setCuts(data.cuts.map((cut, index) => toAppCut(dimensionsToLayout(cut, storageUnits), index)));
        setStepFiles(data.stepFiles ?? []);
        setKerf(layoutDimension(data.kerf, storageUnits, 'in'));
        setPadding(
          typeof data.layoutPadding === 'number' && Number.isFinite(data.layoutPadding)
            ? layoutDimension(data.layoutPadding, storageUnits, 'in')
            : 0.5
        );
        setExcludedKeys(new Set((data.layoutExcludedKeys ?? []) as PartInstanceKey[]));
        setInitialLayoutOverrides(layoutOverridesInUnits((data.layoutOverrides ?? {}) as ManualOverrides, storageUnits, 'in'));
  ...
  const saveProject = async () => {
    if (!project) return;
    const storageUnits = project.units === 'mm' ? 'mm' : 'in';
    setSaving(true);
    try {
      const response = await fetch(`/api/v1/projects/${id}`, {
        ...
        body: JSON.stringify({
          kerf: layoutDimension(kerf, 'in', storageUnits),
          units: storageUnits,
          groupMultipliers,
          layoutOverrides: layoutOverridesInUnits(overrides, 'in', storageUnits),
          layoutExcludedKeys: [...excludedKeys],
          layoutPadding: layoutDimension(padding, 'in', storageUnits),
          layoutHasActive: Boolean(result),
          stepActiveFileId: project.stepActiveFileId ?? null,
          stocks: stocks.map((stock) => {
            const s = dimensionsFromLayout(stock, storageUnits);
            return {
              id: s.dbId, name: s.name, l: s.l, w: s.w, t: s.t, qty: s.qty, mat: s.mat,
            };
          }),
          cuts: cuts.map((cut) => {
            const c = dimensionsFromLayout(cut, storageUnits);
            return {
              id: c.dbId,
      ...
      if (!response.ok) throw new Error('Could not save the layout.');
      setIsDirty(false);
    } catch (error) {
  ...
  const buildLayoutSvgContent = useCallback(() => {
    ...
    result.sheets.forEach((sheet, i) => {
      const context = buildSheetContext(i);
      if (!context) return;
      svgContent += `<text x="${padding}" y="${yOffset + 16}" fill="#334155" font-size="14" font-weight="500">Sheet ${i + 1}: ${sheet.name} (${dim(sheet.w)} × ${dim(sheet.l)} ${unitLabel})</text>`;
      yOffset += 24;
      ...
      for (let tick = 0; tick <= sheet.w; tick += tickInterval) {
        ...
        svgContent += `<text x="${x}" y="${yOffset - 18}" text-anchor="middle" fill="#94a3b8" font-size="9">${dim(tick)}</text>`;
      }
      for (let tick = 0; tick <= sheet.l; tick += tickInterval) {
        ...
        svgContent += `<text x="${padding - 18}" y="${y + 3}" text-anchor="end" fill="#94a3b8" font-size="9">${dim(tick)}</text>`;
      }
      ...
      context.numberedCuts.forEach((c, j) => {
        ...
        svgContent += `<text x="${x + w / 2}" y="${y + h / 2 + 10}" text-anchor="middle" fill="#64748b" font-size="${Math.min(w, h) / 8}">${dim(c.pw)} × ${dim(c.ph)} ${unitLabel}${c.rot ? ' (R)' : ''}</text>`;
      });
    ...
    svgContent += '</svg>';
    return svgContent;
  }, [buildSheetContext, dim, result, unitLabel]);
  ...
  return (
    <div className="flex min-h-full flex-col bg-white xl:h-full xl:overflow-hidden">
        ...
        <details className="group order-0 border-b border-[var(--line)]">
          <summary className="flex cursor-pointer list-none items-center justify-between px-4 py-2.5 [&::-webkit-details-marker]:hidden">
            <div>
              ...
              <p className="text-[10px] text-[var(--muted)]">{dim(kerf)} {unitLabel} kerf · {dim(padding)} {unitLabel} margin</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 text-xs">
            <div>
              ...
              <div className="flex items-center gap-1">
                <LayoutDimensionInput
                  units={units} label="Blade kerf"
                  value={kerf}
                  onChange={setKerf}
                ...
                />
                <span className="text-[10px] text-[var(--muted)]">{unitLabel}</span>
              </div>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-2.5 text-xs">
            <div>
              ...
              <div className="flex items-center gap-1">
                <LayoutDimensionInput
                  units={units} label="Sheet margin"
                  value={padding}
                  onChange={setPadding}
                  className="w-20 rounded border border-[var(--line)] bg-white px-2 py-1 text-xs text-[var(--ink)] outline-none focus:border-[var(--accent)]"
                />
            ...
            <tbody>
              {stocks.map((s) => (
                <tr key={s.id} className="border-t border-[var(--line)]">
                  ...
                  <td className="py-0.5">
                    <LayoutDimensionInput units={units} label={`Length for ${s.name}`} value={s.l} onChange={(value) => setStocks(stocks.map((x) => x.id === s.id ? { ...x, l: value } : x))} className="bg-transparent w-full text-right outline-none text-slate-700 text-xs" />
                  </td>
                  <td className="py-0.5">
                    <LayoutDimensionInput units={units} label={`Width for ${s.name}`} value={s.w} onChange={(value) => setStocks(stocks.map((x) => x.id === s.id ? { ...x, w: value } : x))} className="bg-transparent w-full text-right outline-none text-slate-700 text-xs" />
                  </td>
                  <td className="py-0.5">
                    <LayoutDimensionInput units={units} label={`Thickness for ${s.name}`} value={s.t ?? 0} onChange={(value) => setStocks(stocks.map((x) => x.id === s.id ? { ...x, t: value } : x))} className="bg-transparent w-full text-right outline-none text-slate-700 text-xs" />
                  </td>
                  <td className="py-0.5">
